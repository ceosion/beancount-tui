...
- `editor.py` — write side. Beancount's library is read-only, so all edits are text-level operations on the ledger source: new entries are appended (`append_entry`, directive-type agnostic); edits replace the original entry's source lines in place (`replace_entry`), located via the `filename`/`lineno` metadata Beancount attaches to every parsed entry, with the entry's extent computed by `entry_line_span` (first line plus all following indented lines). User-entered transaction text is validated with the real Beancount parser (`parse_transaction_text`) before it ever touches a file.
...
- `widgets/` — one file per widget: `account_tree.py` (sidebar hierarchy with balances; posts `AccountSelected` messages), `transaction_table.py` (entries for the selected account: transactions, plus open/close/balance/pad/note directives behind the `t` toggle), `transaction_form.py` (modal add/edit form that assembles Beancount source text and rejects it if the parser does), `directive_form.py` (modal raw-text editor for non-transaction directives, same validation path; supports a target-file picker like `transaction_form.py` when creating a new entry in a multi-file ledger), `directive_type_picker.py` (modal list of creatable directive types — open/close/balance/pad/note — shown by the `a` binding before `directive_form.py` opens with a template), `postings_area.py` (the form's postings editor; Tab-completes account names), `filter_bar.py` (the `/` filter input), `confirm_dialog.py` (yes/no modal used by delete), `income_statement.py` (the `i` report modal: Income/Expenses per account with totals and net, over an optional date range).
